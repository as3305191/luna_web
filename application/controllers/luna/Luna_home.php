...
class Luna_home extends MY_Base_Controller {
	...
	public function index() {
		...
		$items = $this -> dao -> find_all_by_luna($data['login_user']->code);
		$data['p'] = count($items);
		$data['page'] = ceil($data['p']/10);
		$data['now'] = 'luna_home';//現在哪頁

		$members_lose_3days = $this -> dao -> query_ajax_by_lunaall($data['login_user']->code);
		...
		$find_all_members = $this -> dao -> query_ajax_by_lunaall($data['login_user']->code);
		...
		$the_fat_rate_change = $this -> dao -> query_ajax_by_lunaall($data['login_user']->code);
		...
		$find_date = $this -> dao -> query_ajax_by_lunaall($data['login_user']->code);
		$map_date = 0;
