...
class Product_trace extends MY_Base_Controller {
	...
	public function index()
	{
		...
		$this->load->view('mgmt/product_trace/list', $data);
	}
	...
	public function get_data() {
		...
		if(
			 ...
			 !empty($data['sn'])
			 	...
			 	$items = $this -> production_service_v2 -> sum_order_record($data);
	...
	function export_all() {
      ...
      $filename = $date."-pruduction-daily-product.csv";
    	...
    	$f = fopen('php://memory', 'w');
			$fields = array(
				...
				iconv("UTF-8","Big5//IGNORE",'工單成品(KG)'),
      ...
      $result = $items = $this -> production_service -> sum_order_record($data);
			...
			foreach($result as $each) {
				...
				if($each -> type == 2) {
					$sum_weight_2 += floatval($each -> sum_weight);
				}
