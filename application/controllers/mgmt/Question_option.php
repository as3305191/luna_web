...
class Question_option extends MY_Mgmt_Controller {
	...
	function export_all_word($id) {
		...
		$table->addRow(4000);
		$table->addCell(10000,null,8)->addText('員工滿意度(A(10分); B(9分); C(8分); D(7分); E(6分))：</w:t><w:br/><w:t xml:space="preserve">'.
		'系統:'.$items[0].','.'制度:'.$items[1].','.'成本:'.$items[2].','.'協助:'.$items[3].','.'彈性:'.$items[4].'</w:t><w:br/><w:t xml:space="preserve">'.
