...
class Question_option extends MY_Mgmt_Controller {
	...
	function export_all_word($id) {
		...
		$white_table->addRow();
		$white_table->addCell(8000,null,6)->addText('');
		$white_table->addCell(4000,null,2)->addText('部門：'.$find_dep_list->name,array('size'=>16),array('align'=>'left'));
		$white_table->addRow();
		$white_table->addCell(8000,null,6)->addText('');
		$white_table->addCell(4000,null,2)->addText('日期：'.date('Y.m.d', strtotime($list->create_time)),array('size'=>16),array('align'=>'left'));
		$table->addRow(1800);
