...
class Question_option extends MY_Mgmt_Controller {
	...
	function export_all_word($id) {
		...
		$white_table->addRow();
		$white_table->addCell(9000,null,8)->addText('');
		$white_table->addCell(3000,null,4)->addText('部門：'.$find_dep_list->name,array('size'=>13),array('align'=>'left'));
		$white_table->addRow();
		$white_table->addCell(9000,null,8)->addText('');
		$white_table->addCell(3000,null,4)->addText('日期：'.date('Y.m.d', strtotime($list->create_time)),array('size'=>13),array('align'=>'left'));
		$table->addRow(1800);
