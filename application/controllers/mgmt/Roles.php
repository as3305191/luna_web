...
class Roles extends MY_Base_Controller {
	...
	public function add_under(){
		...
		$parent_child = $this -> d_dao -> find_all_by('parent_id',$parent_id);

		$name = $this -> get_post('new_name');
		$parent = $this -> d_dao -> find_by_id($parent_id);
		$plevel = $parent -> level;
		$level = $plevel + 1;
	...
	public function add_department(){
		$data = array();
		$data['parent_id'] = '0';
		$pos_num = $this -> d_dao -> find_all_by('level',0);
	...
	public function del_page(){
		$res = array();
		$id = $this -> get_post('id');
		$childs = $this -> d_dao -> find_all_by('parent_id',$id);
		if (!empty($childs)) {
			$res['error_msg'] = '尚有子項目';
		} else {
			$this -> d_dao -> delete($id);
			$res['success'] = true;
		}
	...
	public function downloaddata(){
		...
		$items = $this -> d_dao -> find_by_id($r_id);
