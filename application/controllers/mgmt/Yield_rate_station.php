...
class Yield_rate_station extends MY_Base_Controller {
	...
	public function index()
	{
		...
		$this->load->view('mgmt/yield_rate_station/list', $data);
	}
	...
	public function get_data() {
		...
		if($data['type'] == '0') {
			$this -> session -> set_userdata('yield_rate_station_report_data_type_0', $data);
		} else {
			$this -> session -> set_userdata('yield_rate_station_report_data', $data);
		}

		$items = $this -> production_service -> sum_order_station_record_group_by_station($data);
	...
	public function edit($id) {
		...
		$this->load->view('mgmt/yield_rate_station/edit', $data);
	}
	...
	function export_all() {
      ...
      $newline = "\r\n";
			$date = date('YmdHis');
      $filename = $date."-yield_rate_station.csv";

			$data = $this -> session -> userdata("yield_rate_station_report_data_type_0");
      ...
      $result = $items = $this -> production_service -> sum_order_station_record_group_by_station($data);
			$sum_weight = 0;
			$sum_number = 0;
			...
			// type 1
			$lineData = array(
				iconv("UTF-8","Big5//IGNORE","成品/半成品"),
				"",
				"",
				"",
			);
			...
			$data = $this -> session -> userdata("yield_rate_station_report_data");
      $result = $items = $this -> production_service -> sum_order_station_record_group_by_station($data);
			$sum_weight = 0;
			$sum_number = 0;
			foreach($result as $each) {
