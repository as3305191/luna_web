...
class Index extends MY_Base_Controller {
	...
	public function give_ticket() {
		...
		if(!empty($find_active_sing)&&$find_active_sing->status==0){
			if($find_active_sing->is_stop_rank==0){
				...
				$res['msg'] = '投票已截止，請觀看排行榜。<br> Voting telah ditutup, harap perhatikan peringkatnya\r Bình chọn đã kết thúc, mời các bạn theo dõi bảng xếp hạng';
			}
	...
	public function view_ranking() {
		...
		if(!empty($sing_status_id)){
			...
			$res['msg'] = '比賽目前進行中，請稍候。<br> Kompetisi sedang berlangsung, harap tunggu.\r Cuộc thi hiện đang diễn ra, vui lòng đợi.';
		}
		$this -> to_json($res);
