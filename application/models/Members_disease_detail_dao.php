<?php
class Members_disease_detail_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('members_disease_detail');
	...
	function find_by_parameter($f){
		...
		$this -> db -> join("disease d", "d.id = _m.disease_id", "left");

		// select
		$this -> db -> select('_m.id,_m.member_disease_id,_m.disease_id');
		$this -> db -> select('d.name,d.level');

		if(!empty($f['member_disease_id'])){
	...
	function delete_by_id($id) {
		$this -> db -> from("$this->table_name as _m");
		$this->db->delete('members_disease_detail', array('member_disease_id' => $id));
	}
