<style>
	#dt_list_wrapper {
		border-top: 1px solid #CCCCCC;
	}
	input[disabled] {
	  background-color: #DDD;
		color: #EEE;
	}
</style>
<div class="tab-content">
	<div class="tab-pane active" id="list_page">
		<!-- widget grid -->
		<section id="widget-grid" class="">

			<!-- row -->
			<div class="row">

				<!-- NEW WIDGET START -->
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">

					<!-- Widget ID (each widget will need unique ID)-->
					<div class="jarviswidget">

						<header>


							<div class="widget-toolbar pull-left">
								<label>範圍查詢 <input id="s_multiple" type="checkbox" class="" value="" /></label>
							</div>
							<div class="widget-toolbar pull-left">
								<input id="s_dt" placeholder="請輸入日期" type="text" class="dt_picker" value="<?= date('Y-m-d') ?>" />
							</div>
							<div class="widget-toolbar pull-left" disabled>
								~ <input id="e_dt" disabled placeholder="請輸入日期" type="text" class="dt_picker" value="<?= date('Y-m-d') ?>" />
							</div>

							<div class="widget-toolbar pull-left">
								<div class="btn-group">
									<button onclick="currentApp.doExportAll()" class="btn dropdown-toggle btn-xs btn-warning" data-toggle="dropdown">
										<i class="fa fa-save"></i>匯出
									</button>
								</div>
							</div>

							<!-- <div class="widget-toolbar pull-left">
								<select id="store_rank" class="form-control input-sm">
									<option value="">請選擇級別</option>
									<?php foreach($store_rank as $each): ?>
										<option value="<?= $each -> rank ?>"><?= $each -> rank ?></option>
									<?php endforeach ?>
								</select>
							</div> -->

						</header>

						<!-- widget div-->
						<div>

							<!-- widget edit box -->
							<div class="jarviswidget-editbox">
								<!-- This area used as dropdown edit box -->

							</div>
							<!-- end widget edit box -->

							<!-- widget content -->
							<div class="widget-body no-padding">
								<div>
									<table class="table table-striped table-bordered table-hover">
										<tr>
											<td class="min100">工單數量</td>
											<td><span id="sum_orders" class="t_red"></span></td>
										</tr>

									</table>
								</div>

								<div id="status_filter" class="btn-group" data-toggle="buttons" style="padding: 3px 0 3px 9px;">
								  <label class="btn btn-default btn-xs active">
								    <input type="radio" name="options" id="o_all" autocomplete="off" value="all"> 全部
										 <span class="badge bg-color-red bounceIn animated">0</span>
								  </label>
								  <label class="btn btn-default btn-xs">
								    <input type="radio" name="options" id="o_0" autocomplete="off" value="0"> 未完成
								    <span class="badge bg-color-red bounceIn animated">19</span>
								  </label>
								  <label class="btn btn-default btn-xs">
								    <input type="radio" name="options" id="o_1" autocomplete="off" value="1"> 已完成
								    <span class="badge bg-color-red bounceIn animated">29</span>
								  </label>
								  <label class="btn btn-default btn-xs">
								    <input type="radio" name="options" id="o_2" autocomplete="off" value="-1"> 取消
								    <span class="badge bg-color-red bounceIn animated">99</span>
								  </label>

								</div>
								<br/>


								<table id="dt_list" class="table table-striped table-bordered table-hover" width="100%">
									<thead>
										<tr>
											<th class="min150">開始時間</th>
											<th class="min150">工單編號</th>
											<th class="">狀態</th>
											<th class="">投入</th>
											<th class="">產出</th>
											<th class="">產成率</th>
											<th class="min150">完工時間</th>
										</tr>

									</thead>
									<tbody>
									</tbody>
								</table>

							</div>
							<!-- end widget content -->

						</div>
						<!-- end widget div -->

					</div>
					<!-- end widget -->

				</article>
				<!-- WIDGET END -->

			</div>

			<!-- end row -->

		</section>
		<!-- end widget grid -->
	</div>

	<div class="tab-pane animated fadeIn" id="edit_page">
		<section class="">
			<!-- row -->
			<div class="row">
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="edit-modal-body">

				</article>
			</div>
		</section>
	</div>

</div>
<style>
#dt_list_wrapper .table-responsive {
	width: 100%;
}
</style>
<script type="text/javascript">
	loadScript(baseUrl + "js/class/BaseAppClass.js", function(){
		loadScript(baseUrl + "js/app/daily_report_order/list.js?<?= time() ?>", function(){
			currentApp = new DailyReportOrderAppClass(new BaseAppClass({}));
		});
	});
</script>
