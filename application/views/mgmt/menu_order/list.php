					...
					<?php if($open_menu_count>0): ?>
						...
						<?php else: ?>		
							...
							<?php for ($i=1;$i<count($menu_list);$i++) : ?>
								<button class="btn-light text-light btn_unsuccess menu_btn menu_<?= $menu_list[$i]->id ?>" style="border-radius: 5px; padding: 10px; width: 220px; height: 48px;" onclick="menu_click(<?= $menu_list[$i]->id ?>,<?= $menu_list[$i]->menu_style_id ?>)"><?= isset($menu_list[$i]->timestamp) ? $menu_list[$i]->timestamp.' ': '' ?><?= $menu_list[$i]->menu_name ?></button>
							<?php endfor ?>
						<?php endif?>
					<?php endif?>
