				...
				<div class="row" style="padding:0px 0px 6px 12px;">
					...
					<?php if($open_menu_count>0): ?>
						<?php if($open_menu_count==1): ?>
							<button class="btn_active btn-success text-light  menu_btn menu_<?= $menu_list[0]->id ?>" style="border-radius: 5px; padding: 10px; width: 220px; height: 48px;" onclick="menu_click(<?= $menu_list[0]->id ?>,<?= $menu_list[0]->menu_style_id ?>)"><?= date('m.d',$menu_list[0]->open_date).' ('.$menu_list[0]->date_day.') '.$menu_list[0]->menu_name ?></button>
						...
						<?php else: ?>		
							...
							<?php for ($i=1;$i<count($menu_list);$i++) : ?>
								<button class="btn-light text-light btn_unsuccess menu_btn menu_<?= $menu_list[$i]->id ?>" style="border-radius: 5px; padding: 10px; width: 220px; height: 48px;" onclick="menu_click(<?= $menu_list[$i]->id ?>,<?= $menu_list[$i]->menu_style_id ?>)"><?= date('m.d',$menu_list[$i]->open_date).' ('.$menu_list[$i]->date_day.') '.$menu_list[$i]->menu_name ?></button>
							<?php endfor ?>
						<?php endif?>
					<?php endif?>
