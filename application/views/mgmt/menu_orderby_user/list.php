				...
				if(d) {
					...
					if(d.list.length>0){
						$.each(d.list, function(){
							if(this.open_dep!=='0'){
								$('<option style="position:relative;height: 10px;text-align: center;background-size: 2px;background-repeat: no-repeat;background-position:left;background-image:url("'+img_url+'")" value='+this.id+'>'+this.timestamp+' '+this.menu_name+'</option>'
									).appendTo($menu_name);
							} else{
								$('<option/>', {
									'value': this.id,
