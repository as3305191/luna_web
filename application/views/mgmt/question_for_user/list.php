		...
		<div class="row" style="padding:0px 0px 6px 12px;">
			...
			<?php if(isset($question_option_id_list)): ?>
				<?php foreach ($question_option_id_list as $each) : ?>
					<button class="btn_active btn-success text-light  menu_btn menu_0" style="border-radius: 5px; padding: 10px; width: 220px; height: 48px;" onclick="q_click(<?php $each->question_style_id ?>,<?php $each->id ?>)">問卷1</button>
				<?php endforeach ?>
			<?php endif?>
