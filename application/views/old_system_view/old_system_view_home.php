                  ...
                  <?php if(count($store_list)>0): ?>
                    ...
                    <?php else: ?>	
                      ...
                      <?php for ($i=1;$i<count($store_list);$i++) : ?>                      
                        <button class="text-light btn_unsuccess menu_btn menu_<?= $store_list[$i]->id ?> btn_unsuccess_text" style="border-radius: 5px; padding: 10px; width: 220px; height: 48px;" onclick="menu_click('<?= $store_list[$i]->id ?>','<?= $store_list[$i]->new_img_address ?>')">&nbsp;<?= $store_list[$i]->store ?></button>
                        <?php endfor ?>
                    <?php endif?>
  ...
  var baseUrl = '<?=base_url('')?>';
    function menu_click(id,new_img_address) {
      $('#store_id').val('');
        ...
        $('.menu_btn').addClass('btn_unsuccess btn_unsuccess_text');
        $('.menu_'+id).removeClass('btn_unsuccess btn_unsuccess_text');
