            ...
            <div class="text-center mb-5">
              ...
              <?php foreach ($ticket_array as $each_key => $each_val): ?>
                <?php if ($each_key==$winner): ?>
                  <div class="mb-4">
                    <h4 class="h6"><a href="#winnerModal" role="button" data-toggle="modal" class="btn btn-md u-btn-gradient-v1 g-mr-10 g-mb-15">第1名</a><span class="float-right g-ml-10"><?=$each_val!=='0' ? $each_val:0?></span></h4>
                    <div class="js-hr-progress-bar progress g-height-4 rounded-0">
                      <div class="js-hr-progress-bar-indicator progress-bar" role="progressbar" style="width: <?= $each_val>0 ? intval(($each_val/$all_ticket)*100): 0 ?>%;" aria-valuenow="<?= $each_val>0 ? intval($each_val/$all_ticket)*100 : 0 ?>" aria-valuenow="<?= $each_val>0 ? intval(($each_val/$all_ticket)*100) : 0 ?>" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </div>
                  <?php $i++; ?>
                <?php else : ?>
