var ProductTracev2AppClass = (function(app) {
	app.basePath = "mgmt/product_trace_v2/";
	...
	app.init = function() {
		app.mDtTable = $('#dt_list').DataTable($.extend(app.dtConfig,{
			ajax : {
				...
				data : function(d) {
					...
					d.product_name = $('#s_product_name').val();
		...
		app.mDtTable.on( 'xhr', function () {
		    var json = app.mDtTable.ajax.json();
				...
				$.each(json.items, function(){
						sumWeight_0 += parseFloat(this.actual_weight);

						sumWeight_2 += parseFloat(this.sum_weight);
						sumWeight_3 = parseFloat(sumWeight_0/sumWeight_2);
				...
				$('#sum_weight_3').html(numberWithCommas(sumWeight_3.toFixed(2)));
		...
		$('#s_product_name').on('change keyup', function(){
			app.tableReload();
		});
