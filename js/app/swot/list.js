var SwotAppClass = (function(app) {
	...
	app.init = function() {
		...
		app.doEdit = function(id,title=0,style=0,dep=0) {
			...
			$('#edit_page').addClass('active');	
				$('#edit-modal-body').load(baseUrl + app.basePath + 'edit/' + id+'?title='+title+'&style='+style+'&dep='+dep+'&type=0', function(){
		...
		app.doEdit1 = function(id,title=0,style=0,dep=0) {
			...
			$('.tab-pane').removeClass('active');
			$('#edit_page').addClass('active');	
				$('#edit-modal-body').load(baseUrl + app.basePath + 'edit/' + id+'?title='+title+'&style='+style+'&dep='+dep+'&type=1', function(){
					// window.location.hash = app.basePath + 'edit/' + id;
					$("#btn-submit-edit").prop( "disabled", false);
					loading.remove();
