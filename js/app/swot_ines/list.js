var SwotinesAppClass = (function(app) {
	app.basePath = "mgmt/swot_ines/";
	...
	app.init = function() {
		...
		app.doEdit = function(id,title=0,style=0,dep=0) {
			...
			$('#edit_page').addClass('active');	
				$('#edit-modal-body').load(baseUrl + app.basePath + 'edit/' + id+'?title='+title+'&style='+style+'&dep='+dep, function(){
		...
		app.do_remove = function() {
			var url = baseUrl + 'mgmt/swot_ines/do_remove'; 
			$.ajax({
				type : "POST",
				url : url,
				data : {
