...
imgmap.prototype.setup = function(config) {
	...
	if (config && config.pic_container) {
		this.pic_container = config.pic_container;
...
imgmap.prototype.setMapHTML = function(map) {
	...
	var shape, coords, href, alt, title, target, id, areaId, areaClass;
	for (var i=0, le = newareas.length; i<le; i++) {
		...
		if (target)
			this.areas[id].atarget = target.toLowerCase();

		areaId = newareas[i].getAttribute('id');
		if (areaId)
			this.areas[id].id = areaId;

		areaClass = newareas[i].getAttribute('class');
		if (areaClass)
			this.areas[id].className = areaClass;
